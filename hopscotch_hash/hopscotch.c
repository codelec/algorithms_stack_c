...
{
	uint32_t hash , segment , start_bucket_id , synergy , mask , free_distance , i;
	uint32_t *start_hop_info;
	BUCKET *start_bucket , *free_bucket;
	bool flag1 , flag2 , flag3 , flag4 = 0;
	start_again : //label
	flag1 = 0 , flag2 = 0 , flag3 = 1 , mask = 1 ;//initial declarations
	hash = hashlittle(key,sizeof(*key),hash_init_val);
	if(flag4)
		hash_init_val--;
	...
	*of the current row provided that the next row
	...
	*flag4 - to prevent hashing to the bottom right corner cell of the 2d 
	*/
	...
	*/
	...
	if (free_distance < HOP_SIZE){
		...
		(*start_hop_info) |= mask; //update hop_info with the recent allocation
	}
	else{
		if(flag2)
...
{
	uint32_t hash,segment,start_bucket_id,synergy,mask,count;
	uint32_t *start_hop_info;
	bool flag1 , flag2 , flag4 = 0;
	check_again:
	flag1 = 0 , flag2 = 0 , mask = 1 , count = 0;
	hash = hashlittle(check_key,sizeof(*check_key),hash_init_val);
	if(flag4)
		hash_init_val--;
	start_bucket_id = hash & bucket_mask ;
	segment = ((hash & segment_mask)>>BUCKET_MASK_BITS) ;
	...
	/*label
	*the following lines are to make sure that the element which was originally
	*hashed to the bottom rigth corner(already allocated) of the 2d array , which is then rehashed to 
	*another location could be found
	*/
	if (flag1 && ((start_bucket_id + 1) == ADDR_RANGE)){
		flag4 = 1;
		hash_init_val++;
		goto check_again;
	}
...
void resize()
{
	...
	if ((current_max_segment-1)!= 0)
		segment_mask = (segment_mask << 1) + bucket_mask + 1;//100 -> 1100 -> 11100 -> 111100 
	else
