void bfs(vertex* s)
{
	...
	s -> level = 1;
	head -> node = s;
	head -> next_connection = (list*)malloc(sizeof(list));//giving first memory space to the pointer
	tail = head -> next_connection;//assiging it to tail only after memory space is alloted
	while(head != tail)
	{	
